<!-- EDIT Vacancy Modal -->
<div x-data="{ open: false, vacancy: {} }" @open-modal.window="if ($event.detail.modal === 'edit-vacancy') { open = true; vacancy = $event.detail.vacancy; }" x-cloak>
    <!-- Modal Overlay -->
    <div x-show="open"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full text-sm max-w-3xl transform transition-all duration-300 scale-95 border border-gray-200"
            ...
            <!-- Modal Header -->
            <div class="flex justify-between items-center px-6 py-1 bg-green-600 text-white rounded-t-2xl">
                <h2 class="text-base font-medium flex items-center gap-2">
                    <i class="fas fa-briefcase text-white"></i> Edit Vacancy
                </h2>
            ...
            <!-- Modal Form -->
            <form method="POST" :action="'/vacancies/' + vacancy.id" class="p-4 space-y-2">
                @csrf
                @method('PUT')
                <input type="hidden" name="user_id" :value="vacancy.user_id">

                <p class="text-sm text-gray-700">
                    Update the details below to edit the vacancy.
                </p>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div>
                        <x-input-label for="refno" :value="__('Reference No.')" />
                        <x-text-input id="refno" name="refno" type="text"
                            class="w-full text-sm" x-model="vacancy.refno" required />
                    ...
                    <div>
                        ...
                        <x-text-input id="position" name="position" type="text"
                            class="w-full text-sm" x-model="vacancy.position" required />
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select id="job_grade" name="job_grade"
                            ...
                            x-model="vacancy.job_grade" required>
                ...
                <div class="pt-1 space-y-4">
                    <div>
                        ...
                        <textarea id="requirements" name="requirements"
                            class="w-full text-sm border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            rows="5" x-model="vacancy.requirements" required></textarea>
                    ...
                    <div>
                        ...
                        <textarea id="duties" name="duties"
                            class="w-full text-sm border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            rows="5" x-model="vacancy.duties" required></textarea>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2">
                    <div>
                        ...
                        <x-text-input id="application_deadline" name="application_deadline" type="date"
                            class="w-full text-sm" x-model="vacancy.application_deadline" required />
                    ...
                    <div>
                        ...
                        <select id="status" name="status"
                            class="w-full text-sm border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            x-model="vacancy.status" required>
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    ...
                    <button type="submit"
                        class="bg-green-600 hover:bg-green-700 text-white px-2 py-1 rounded-md flex items-center gap-2 shadow-md hover:shadow-lg transition-all">
