                    ...
                    <table class="w-full bg-white border border-gray-300 rounded-lg text-sm">
                        <thead
                            ...
                            <tr>
                                ...
                                <th class="p-3 text-left">Salary (KSh)</th>
                                <th class="p-3 text-left">Start Date</th>
                                <th class="p-3 text-left">End Date</th>
                                <th class="p-3 text-left">Responsibilities</th>
                            ...
                            @forelse ($employments as $employment)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="p-3 font-medium text-gray-800 whitespace-nowrap">
                                        <i class="fas fa-building text-gray-500 mr-1"></i>{{ $employment->company }}
                                    </td>
                                    <td class="p-3 text-gray-700 whitespace-nowrap">{{ $employment->designation }}</td>
                                    <td class="p-3 text-gray-700 whitespace-nowrap">KSh
                                        {{ number_format($employment->salary) }}</td>
                                    <td class="p-3 text-gray-600 whitespace-nowrap">
                                        {{ \Carbon\Carbon::parse($employment->start_date)->format('M Y') }}</td>
                                    <td class="p-3 text-gray-600 whitespace-nowrap">
                                    ...
                                    <td class="p-3 text-gray-700 whitespace-nowrap">
                                        {{ Str::limit($employment->responsibilities, 50) }}
                                    </td>
                                    <td class="p-3 text-center">
                                        <div class="flex justify-center items-center gap-4">
                                            <!-- Edit Button -->
                                            <button
                                                @click="$dispatch('open-modal', { modal: 'edit-employment', employment: {{ json_encode($employment) }} })"
                                                class="text-blue-500 hover:text-blue-700 flex items-center gap-1 transition-all duration-200">
                                                <i class="fas fa-edit"></i> Edit
                                            </button>

                                            <!-- Delete Button -->
                                            <form id="delete-form-{{ $employment->id }}"
                                                action="{{ route('employment.destroy', $employment->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" onclick="confirmDelete({{ $employment->id }})"
                                    ...
                                    <td colspan="8" class="p-4 text-center">
                                        <div
                                            ...
                                            <p class="mt-1 text-sm">
                                                No Employment History Added Yet. Click <strong>Add Employment</strong>
                                            </p>
                                        </div>
                                    </td>
                                </tr>
                    ...
                    @forelse ($employments as $employment)
                        <div class="bg-white p-4 shadow-lg rounded-lg border border-gray-200">
                            <div>
                                ...
                                <p class="text-sm text-gray-700 flex items-center gap-1">
                                    <i class="fas fa-money-bill-wave text-green-500"></i> KSh
                            ...
                            <div class="mt-4 flex justify-between border-t pt-3">
                                ...
                                <form id="delete-form-{{ $employment->id }}"
                                    action="{{ route('employment.destroy', $employment->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button"
                                        class="text-red-500 hover:text-red-700 flex items-center gap-1"
                    ...
                    @empty
                        <div class="bg-blue-100 border border-blue-300 text-blue-700 px-4 py-3 rounded-lg shadow-md">
                            ...
                            <p class="mt-5 text-sm items-center text-center">
                                {{ __('No employment history added yet.Click Add') }} </p>
                        </div>
                    @endforelse
                </div>
