...
class CountiesSubcountiesandWards extends Seeder
{
    /**
    ...
    {
        // Check if the counties table is empty
        if (DB::table('counties')->count() > 0) {
            $this->command->info('Counties table already seeded. Skipping...');
            return;
        }
        // Check if the sub_counties table is empty
        if (DB::table('sub_counties')->count() > 0) {
            $this->command->info('Subcounties table already seeded. Skipping...');
            return;
        }
        // Check if the wards table is empty
        if (DB::table('wards')->count() > 0) {
            $this->command->info('Wards table already seeded. Skipping...');
            return;
        }
         // Seed All Counties
         $counties = [
       ...
       DB::table('counties')->insert($counties);
        ...
        $this->command->info('Counties , Subcounties and Wards seeded successfully!');
    }
